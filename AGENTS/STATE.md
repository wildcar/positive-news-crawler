...
- Migration `0006_newstranslation` is ready but not deployed to production.
...
- Deploy with `scripts/update-ubuntu.sh`, add `NEWSCRAWLER_ROUTER_AUTH_TOKEN` and translation settings to `/etc/newscrawler/newscrawler.env`, then smoke-test translation through the live operator UI.
